...
class Mf_payment_packaging_model extends CI_Model
{
	...
	public function getSupplierDeu($id)
	{
		...
		$q = $this->db->get_where('mf_purchases_packaging', array('supplier_id' => $id));
	...
	{
		$total = 0;
		$this->db->select('mf_purchases_packaging.total, mf_purchases_packaging.paid, mf_purchases_packaging.deu');
	...
	public function advPayAmount($type, $id = NULL, $date = null, $store_id = NULL)
	{
		...
		$this->db->select('packaging_adv_payment.adv_amount');
		$this->db->from('packaging_adv_payment');
		if ($id != '') {
			$this->db->where('suppliers_id', $id);
		}

		if ($date != '') {
			$this->db->like('add_date', $date);
		}
		$query = $this->db->get();
	...
	{
		if ($this->db->insert('mf_purchase_payments', $data)) {
	...
	public function UpdateSupplierDeu($data, $id)
	{
		...
		if ($this->db->update('mf_purchases_packaging', $data, array('id' => $id))) {
			return true;
		}
		return false;
	}
	...
	{
		$this->db->insert('mf_payment_packaging', $data);
	...
	{
		$q = $this->db->get_where('mf_purchase_payments', array('todayP_Payment' => $id));
		if ($q->num_rows() > 0) {
			return  $q->result();
		}
		return FALSE;
	...
	public function deletePaymentForPurchase($id)
	{
		...
		if ($this->db->delete('mf_purchase_payments', array('todayP_Payment' => $id))) {
	...
	public function deleteAdvSupPay($id)
	{
		...
		if ($this->db->delete('packaging_adv_payment', array('mf_payment_id' => $id))) {
	...
	{
		if ($this->db->delete('expenses', array('mf_payment_id' => $id))) {
			return true;
		}
		return FALSE;
	}
	...
	{
		if ($this->db->insert('packaging_adv_payment', $data)) {
			return true;
		}
		return false;
	}
