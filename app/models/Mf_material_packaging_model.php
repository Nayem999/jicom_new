...
class mf_material_packaging_model extends CI_Model
{
	...
	{
		if($this->db->insert('mf_material_packaging', $data)) {
	...
	{
		if($this->db->update('mf_material_packaging', $data, array('id' => $id))) {
			return true;
		}
		return false;
	}
	...
	{
		if($this->db->delete('mf_material_packaging', array('id' => $id))) {
			return true;
		}
		return FALSE;
	}
	...
	public function getMaterialLaserBySid($supplier){ 
        ...
        $q = $this->db->get_where('mf_material_packaging', array('id' => $supplier));
