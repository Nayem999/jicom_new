...
class Mf_purchases_packaging_model extends CI_Model
{
    ...
    public function getPurchaseByID($id) {
        ...
        $q = $this->db->get_where('mf_purchases_packaging', array('id' => $id), 1);
	...
	public function purchasesAmount($type, $id =NULL,$date=null,$store_id=NULL){		
		 $total = 0;
		 $this->db->select('mf_purchases_packaging.*');
		 $this->db->from('mf_purchases_packaging');
    ...
    public function getAllPurchaseItems($purchase_id) { 
        ...
        $this->db->select('mf_purchase_packaging_material.*, mf_material_packaging.name as product_name, mf_unit.name as unit_name')

            ->join('mf_material_packaging', 'mf_material_packaging.id=mf_purchase_packaging_material.material_id', 'left')

            ->join('mf_unit', 'mf_unit.id=mf_material_packaging.uom_id', 'left')

            ->group_by('mf_purchase_packaging_material.id')

            ->order_by('id', 'asc');

        $q = $this->db->get_where('mf_purchase_packaging_material', array('purchase_id' => $purchase_id));

        $purchases = $this->db->get_where('mf_purchases_packaging', array('id' => $purchase_id)); 
		$transport_cost=0;
    ...
    public function addPurchase($data, $items) {  
        ...
        if ($this->db->insert('mf_purchases_packaging', $data)) {
            ...
            foreach ($items as $item) {
                ...
                if($this->db->insert('mf_purchase_packaging_material', $item)) {
					...
					if($incstoreqty){
						...
						$this->db->insert('mf_material_packaging_store_qty', $insertdata);
    ...
    public function updatePurchase($id, $data = NULL, $items = array()) {
        ...
        if ($this->db->update('mf_purchases_packaging', $data, array('id' => $id)) && $this->db->delete('mf_purchase_packaging_material', array('purchase_id' => $id))) {
    ...
    public function getProductNames($term, $limit = 10) {
        ...
        $q = $this->db->get('mf_material_packaging');
...
public function deletePaymentForPurchase($id) {
	...
	public function getStoreId_purchases($id){
       ...
       $this->db->select('mf_purchases_packaging.store_id'); 
       $this->db->where(array('id' => $id));
       $q = $this->db->get('mf_purchases_packaging');
        if ($q->num_rows() > 0) {
        	$result =  $q->row();
        	 return $result;	
        }
    ...
    function storeProQtyDelete($item_id,$item_quantity,$store_id,$brand_id,$item_cost){
 	...
 	function storeProQtyUpdate($item_id,$item_quantity,$store_id,$brand_id,$item_cost){
        $q = $this->db->get_where('mf_material_packaging_store_qty', array('material_id' => $item_id , 'store_id' => $store_id, 'brand_id' => $brand_id ), 1);
        if ($q->num_rows() > 0) {
	        $value =  $q->row();
	        ...
	        if($this->db->update('mf_material_packaging_store_qty', $data, array('material_id'=>$item_id, 'store_id'=>$store_id, 'brand_id'=>$brand_id))){
	            return true;
	        }
	        return false; 
    	}else{
			...
			$this->db->insert('mf_material_packaging_store_qty', $sqdata);
