...
class Mf_payment_model extends CI_Model
{
	...
	public function getSupplierDeu($id)
	{
		...
		$q = $this->db->get_where('mf_purchases', array('supplier_id' => $id));
	...
	{
		$total = 0;
		$this->db->select('mf_purchases.total, mf_purchases.paid, mf_purchases.deu');
	...
	public function advPayAmount($type, $id = NULL, $date = null, $store_id = NULL)
	{
		...
		$this->db->select('mf_adv_payment.adv_amount');
		$this->db->from('mf_adv_payment');
		if ($id != '') {
	...
	public function UpdateSupplierDeu($data, $id)
	{
		...
		if ($this->db->update('mf_purchases', $data, array('id' => $id))) {
			return true;
		}
		return false;
	}
	...
	{
		$this->db->insert('mf_payment', $data);
	...
	{
		if ($this->db->insert('mf_adv_payment', $data)) {
			return true;
		}
		return false;
	}
