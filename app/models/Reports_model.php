...
class Reports_model extends CI_Model
{
    ...
    public function topProducts($store_id)
    {
        ...
        if ($store_id != NULL) {
            $this->db->where('sale_items.store_id', $store_id);
        }
    ...
    public function saleAndPurseCount($store_id, $start_date, $end_date)
    {
        if ($store_id || $start_date || $end_date) {
            $filter_conditions = $filter_conditions2 = $filter_conditions3 = '';
            if ($store_id) {
                $filter_conditions .= " and sa.store_id= $store_id ";
                $filter_conditions2 .= " and pur.store_id= $store_id ";
                $filter_conditions3 .= " and rt.store_id= $store_id ";
            }
            if ($start_date) {
                $filter_conditions .= " and ss.date >= '$start_date' ";
                $filter_conditions2 .= " and pp.date >= '$start_date' ";
                $filter_conditions3 .= " and sr.date_submit >= '$start_date' ";
            }
            if ($end_date) {
                $filter_conditions .= " and ss.date <= '$end_date' ";
                $filter_conditions2 .= " and pp.date <= '$end_date' ";
                $filter_conditions3 .= " and sr.date_submit <= '$end_date' ";
            }

            $query = $this->db->query("SELECT PR.id,PR.name,code,( SELECT COALESCE(sum(sa.quantity), 0) FROM tec_sale_items sa, tec_sales ss WHERE sa.product_id=PR.id and sa.sale_id=ss.id $filter_conditions ) sold,
                        ( SELECT COALESCE(sum(pur.quantity), 0)
                          FROM tec_purchase_items pur, tec_purchases pp
                          WHERE pur.product_id=PR.id and pur.purchase_id=pp.id $filter_conditions2) purchase,
                          ( SELECT COALESCE(sum(rt.return_qty), 0)
                          FROM tec_sreturn_items rt, tec_salesreturn sr WHERE rt.product_id=PR.id and rt.sreturn_id=sr.sreturn_id $filter_conditions3) sreturn 
    ...
    public function pquery($type, $code, $warehouse = NULL)
    {
        ...
        if ($q->num_rows() > 0) {
            ...
            if ($type == 'sales') {
                ...
                $this->db->where('store_id', $warehouse);
                if ($saleidin != '') {
                    $this->db->where_not_in('sale_id', $saleidin);
                }
                $query = $this->db->get();;
                $results = $query->result();
                return $results;
    ...
    public function invoiceProfit($store_id = 0)
    {
        ...
        if (!$this->Admin) {
            ...
            if ($store_id) {
                $filterStore = " WHERE PR.store_id=$store_id ";
            } else {
                $filterStore = '';
            }
    ...
    public function salaryReport($start_date, $end_date)
    {
        ...
        if ($start_date) {
            $this->db->where('pay_date >=', $start_date . ' 00:00:00');
        }
        if ($end_date) {
            $this->db->where('pay_date <=', $end_date . ' 23:59:59');
        }
        if (!$this->Admin) {
    ...
    public function bankPays($start_date = NULL, $end_date = NULL, $store_id = 0)
    {
        ...
        if ($store_id) {
            $this->db->where('tranjiction.store_id', $store_id);
        }

        if (!$this->Admin) {
            $this->db->where('tranjiction.store_id', $this->session->userdata('store_id'));
        }
    ...
    public function totalBankCash2($bank_id = NULL, $end_date = NULL, $start_date = NULL, $store_id = 0)
    {
        ...
        $this->db->from('bank_account');
        if ($store_id) {
            $this->db->where('bank_account.store_id', $store_id);
        }
    ...
    public function supplierPayment($start_date = NULL, $end_date = NULL, $store_id = 0)
    {
        ...
        if ($store_id) {
            $this->db->where('today_purchase_payment.store_id', $store_id);
        }
        if (!$this->Admin) {
            $this->db->where('today_purchase_payment.store_id', $this->session->userdata('store_id'));
        }
        $query = $this->db->get();
        return $query->result();
    }
