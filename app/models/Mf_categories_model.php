...
class Mf_categories_model extends CI_Model
{
    ...
    public function addCategory($data) {
        if ($this->db->insert('mf_categories', $data)) {
            return true;
        }
        return false;
    }
    ...
    public function add_categories($data = array()) {
        if ($this->db->insert_batch('mf_categories', $data)) {
            return true;
        }
        return false;
    }
    ...
    public function updateCategory($id, $data = NULL) {
        if ($this->db->update('mf_categories', $data, array('id' => $id))) {
            return true;
        }
        return false;
    }
    ...
    public function addExpCategory($data) {
        if ($this->db->insert('mf_expens_category', $data)) {
    ...
    public function getAllCategories() {
        $this->db->from('mf_expens_category');
    ...
    public function getCategoryByID($id) {
        $q = $this->db->get_where('mf_expens_category', array('cat_id' => $id));
            if ($q->num_rows() > 0) {
                return $q->result();
            }
        return FALSE;
    ...
    public function updateExpCategory($id, $data = NULL) {
        if ($this->db->update('mf_expens_category', $data, array('cat_id' => $id))) {
            return true;
        }
        return false;
    }
    ...
    public function deleteExpCategory($id) {
        if ($this->db->delete('mf_expens_category', array('cat_id' => $id))) {
            return true;
        }
        return FALSE;
    }
    public function likeAsValue() {
        ...
        $q = $this->db->query("SELECT * FROM tec_mf_categories where parent_id =".$q->row()->p_count);
        $results = $q->result(); 

        if($results==Array ( )) { 
             
            $q = $this->db->query("SELECT * FROM tec_mf_categories where id =".$pcatid); 
            $results = $q->result();
   ...
   public function getIdByParen($id){ 
    ...
    public function getCategories($store_id=0) {
        ...
        $this->db->select(
            $this->db->dbprefix('mf_categories').'.id,'.
            $this->db->dbprefix('mf_categories').'.code,'.
            $this->db->dbprefix('mf_categories').'.image,'.
            $this->db->dbprefix('mf_categories').'.name,'.
            $this->db->dbprefix('mf_categories').'.parent_id,SUM('. 
            $this->db->dbprefix('product_store_qty').'.quantity) as qty');       
        $this->db->from('mf_categories');
        $this->db->join('products', 'products.category_id=mf_categories.id');  
        $this->db->join('product_store_qty', 'products.id=product_store_qty.product_id');  
        $this->db->group_by('mf_categories.id');
    ...
    function getSubcategories($cat_id) {
        $q = $this->db->get_where('mf_categories', array('id' => $cat_id), 1); 
