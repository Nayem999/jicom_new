...
<?php
    ...
    function get_products()
    {
        ...
        if ($start_date) {
            $this->datatables->where('sale_items.date >=', $start_date . ' 00:00:00');
        }
        if ($end_date) {
            $this->datatables->where('sale_items.date <=', $end_date . ' 23:59:59');
        }
        if ($store_id != NULL) {
    ...
    public function get_store_product_stock()
    {
        ...
        if ($warehouse) {
            $this->datatables->where('store_id', $warehouse);
        }
        if (!$this->Admin) {
