...
class Reports extends MY_Controller
{
    ...
    function expenses_rpt()
    {
        ...
        if ($employee_id) {
            $this->db->where('expenses.employee_id', $employee_id);
        }
        if ($start_date) {
    ...
    function get_products()
    {
        ...
        $this->datatables
            ->select($this->db->dbprefix('products') . ".name, " .
                $this->db->dbprefix('stores') . ".name as storename , GROUP_CONCAT(" .
                $this->db->dbprefix('sale_items') . ".sale_id ORDER BY tec_sale_items.sale_id desc) as invoice_id , " .
        ...
        if (!$this->Admin) {
            ...
            if ($store_id != NULL) {
                $this->datatables->where('sale_items.store_id', $store_id);
            }
        }
    ...
    function get_excel_products($data = '')
    {
        ...
        if ($store_id != NULL) {
            $sval = $this->site->whereRow('stores', 'id', $store_id);
            $fields = array('STORE NAME', $sval->name);
            $excelData .= implode("\t", array_values($fields)) . "\n";
        }
        if ($customer != NULL) {
            $cval = $this->site->whereRow('customers', 'id', $customer);
            $fields = array('CUSTOMER NAME', $cval->name);
            $excelData .= implode("\t", array_values($fields)) . "\n";
        }


        $fields = array('NAME', 'STORE NAME', 'INVOICE ID', 'CODE', 'SOLD', 'TAX', 'COST', 'INCOME', 'PROFIT');
        $excelData .= implode("\t", array_values($fields)) . "\n";

        if (count($query_data) > 0) {
            foreach ($query_data as $row) {
                $lineData = array($row->name, $row->storename, $row->invoice_id, $row->code, $row->sold, $row->tax, $row->cost, $row->income, $row->profit);
                $excelData .= implode("\t", array_values($lineData)) . "\n";
            }
        } else {
            $excelData .= 'No records found...' . "\n";
    ...
    public function pettycashview()
    {
        ...
        if ($start_date) {
            $this->datatables->where('entry_date >=', $start_date);
        }
        if ($end_date) {
            $this->datatables->where('entry_date <=', $end_date);
        }
    ...
    public function receivablelist()
    {
        ...
        $this->data['recivabl'] = $this->reports_model->recablelist($cID, $sID);
