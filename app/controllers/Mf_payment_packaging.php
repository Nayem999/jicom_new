...
class Mf_payment_packaging extends MY_Controller
{
    ...
    function __construct() {
        ...
        $this->load->model('mf_payment_packaging_model');
    ...
    public function add() { 
      if(!$this->site->route_permission('mf_payment_packaging_add')) {
        ...
        $this->data['page_title'] = 'Packaging Material Payment';
        
        $bc = array(
            array(
                'link' => '#',
                'page' => 'Packaging Material Payment'
            )
        );
        
        $meta = array(
            'page_title' => 'Packaging Material Payment',
            'bc' => $bc
        );
        
        $this->page_construct('mf_payment_packaging/add', $this->data, $meta);
    }
    public function supplierInfo($id){

      $suppliers = $this->mf_payment_packaging_model->getSupplierByID($id);
      $SupPro = $this->mf_suppliers_model->getSupplierByID($id);   
      
      $today = date("Y-m-d");
      $total = $deu = $paid = 0;

      $todayTotalPurchases = $this->mf_payment_packaging_model->purchasesAmount('total',$id,$today);
      $todayPaidpurchases = $this->mf_payment_packaging_model->purchasesAmount('paid',$id,$today);
      $todayDeupurchases = $this->mf_payment_packaging_model->purchasesAmount('deu',$id,$today);
      $todayAdAmount = $this->mf_payment_packaging_model->advPayAmount('adv_amount',$id,$today);
      $totalAdAmount = $this->mf_payment_packaging_model->advPayAmount('adv_amount',$id);
    ...
    public function today_payment(){
      ...
      $getSupplierID = $this->mf_payment_packaging_model->getSupplierByID($supplierid);
      ...
      foreach ($getSupplierID as $key => $SupplierID);
        ...
        if($type=='Cheque' || $type=='TT'){
          if(($bankID=='')||($cheque_no=='')){
            ...
            redirect('mf_payment_packaging/purchase_payment');
          } else { 
          }
        }     
        $date       = date('Y-m-d');
        ...
        $paid       = 0;
        $totalDeu 	= 0;
        $suppliers = $this->mf_payment_packaging_model->getSupplierDeu($supplierid);
        foreach ($suppliers as $key => $value) {
        ...
        $todayPurchasePayment = $this->mf_payment_packaging_model->payPayment($payPaymentdata);
        ...
        foreach ($suppliers as $key => $value) {
           ...
           if(($total > $paid) && ($paidamount  > 0) ){
                if($paidamount < $value->deu ){                   
                    ...
                    $this->mf_payment_packaging_model->UpdateSupplierDeu($data,$value->id);
                    $addPayData = array(
                        'date' => date('Y-m-d H:i'),
                    ...
                    $this->mf_payment_packaging_model->addPayment($addPayData);
        ...
        $this->mf_payment_packaging_model->addAdvPayment($advData);
        $odlNewPaid = $value->paid + $value->deu;
        $total      = 0;
        if($type=='Cheque'){
	        $bankPending = array(
        ...
        $expens_cat=$this->site->whereRow('expens_category','name','Packaging Material Payment');
        if($expens_cat)
        {
        ...
        {
          $add_expens_category = array('code' => 'PMP', 'name' => 'Packaging Material Payment');
      ...
      $this->session->set_flashdata('message', lang('Packaging Payment submited successfully'));
      $this->add();
    }
    ...
    public function todayPurchases() {
       ...
       $this->data['page_title'] = 'Collection';        
        ...
        $this->page_construct('mf_payment_packaging/todayPurchaseslist', $this->data, $meta);
    }
    ...
    public function paymentlist(){ 
        ...
        $this->datatables->select($this->db->dbprefix('mf_payment_packaging') . ".id as id, " . 
        	$this->db->dbprefix('mf_suppliers') . ".name as name, " .  
        	$this->db->dbprefix('mf_payment_packaging') . ".payment_date as payment_date ,  " . 
        	$this->db->dbprefix('mf_payment_packaging') . ".payment_amount, " .
        	$this->db->dbprefix('mf_payment_packaging') . ".type, " .
        	$this->db->dbprefix('mf_payment_packaging') . ".cheque_no, " . 
        	$this->db->dbprefix('mf_payment_packaging') . ".payment_note", FALSE);        
        $this->datatables->join('mf_suppliers', 'mf_suppliers.id=mf_payment_packaging.supplier_id');
        $this->datatables->join('stores', 'mf_suppliers.store_id=stores.id');  

        if(!$this->Admin){
          $this->datatables->where('mf_payment_packaging.store_id',$this->session->userdata('store_id'));
        }

        $this->datatables->add_column("Actions", "<div class='text-center'><div class='btn-group'>
                
         <a href='" . site_url('mf_payment_packaging/delete/$1') . "' onClick=\"return confirm('". lang('You are going to delete payment , please click ok to delete') ."')\" title='".lang("delete_payment")."' class='tip btn btn-danger btn-xs'><i class='fa fa-trash-o'></i></a></div></div>", "id"); 
        
        $this->datatables->from('mf_payment_packaging'); 
    ...
    public function index() { 
      if(!$this->site->route_permission('mf_payment_packaging_view')) {
        $this->session->set_flashdata('error', lang('access_denied'));
        redirect();
      }
       ...
       $this->data['suppliers'] = $this->site->getAllMfSuppliers();      
       $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));        
       $this->data['page_title'] = 'Packaging Material Payment List';        
        $bc = array(
            array(
                'link' => '#',
                'page' => 'Packaging Material Payment List'
            )
        );        
        $meta = array(
            'page_title' => 'Packaging Material Payment List',
            'bc' => $bc
        );    
        $this->page_construct('mf_payment_packaging/index', $this->data, $meta);
    }

    public function delete($id = null){
       
      $getSalesIdfpaytb = $this->mf_payment_packaging_model->getAllpaymentID($id);

      foreach ($getSalesIdfpaytb as $key => $value) {
         $saleAmountptb = $value->amount ;
        
         $getSalesIdfSaltb = $this->mf_payment_packaging_model->getPurchaseByID($value->mf_purchases_id);
         ...
         $this->mf_payment_packaging_model->UpdateSupplierDeu($dataUpdate,$getSalesIdfSaltb->id); 
         $this->mf_payment_packaging_model->deletePaymentForPurchase($id);
         $this->mf_payment_packaging_model->deleteTodayPurPay($id);
      ...
      $this->mf_payment_packaging_model->deleteAdvSupPay($id);
      $this->mf_payment_packaging_model->deleteExp($id);
      $this->index();
    }
}