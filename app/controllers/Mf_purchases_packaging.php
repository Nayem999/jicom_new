...
class Mf_purchases_packaging extends MY_Controller
{
    ...
    function __construct() {
        ...
        $this->load->model('mf_purchases_packaging_model');
    ...
    function index() { 
        ...
        $this->page_construct('mf_purchases_packaging/index', $this->data, $meta);
    ...
    function get_purchases() { 
        ...
        $this->datatables->select($this->db->dbprefix('mf_purchases_packaging') . ".id as id, " . 
                      
            $this->db->dbprefix('mf_purchases_packaging') . ".date as date , supplier_id," .  

            $this->db->dbprefix('mf_suppliers') . ".name as cname , total, paid , deu ", FALSE);
        
        $this->datatables->join('mf_suppliers', 'mf_suppliers.id=mf_purchases_packaging.supplier_id','left');
        $this->datatables->join('stores', 'stores.id=mf_purchases_packaging.store_id');
        $this->datatables->from('mf_purchases_packaging');
        $this->datatables->group_by('mf_purchases_packaging.id');
		...
		if($this->site->route_permission('mf_purchases_packaging_delete')) {
			$action.="<a href='" . site_url('mf_purchases_packaging/delete/$1') . "' onClick=\"return confirm('" . lang('alert_x_purchase') . "')\" title='" . lang("delete_purchase") . "' class='tip btn btn-danger btn-xs'><i class='fa fa-trash-o'></i></a>";
		}
        $action.="</div></div>";
        ...
        if($store_id){$this->datatables->where('mf_purchases_packaging.store_id',$store_id);}
    ...
    function view($id = NULL) { 
        ...
        $this->data['mf_purchase'] = $this->mf_purchases_packaging_model->getPurchaseByID($id);
        
        $supplier_id = $this->data['mf_purchase']->supplier_id;
        
        $this->data['s_name'] = $this->mf_purchases_packaging_model->getSupplierInfo($supplier_id)->name;
        
        $rows = $this->mf_purchases_packaging_model->getParchasePay($id);
        if ($rows == FALSE) {
            $totla_pay = 0;
        } else {
            $totla_pay = 0;
        ...
        $pay = $this->mf_purchases_packaging_model->getParInfo($id);
        
        $this->data['pay'] = $pay;
        
        $this->data['items'] = $this->mf_purchases_packaging_model->getAllPurchaseItems($id);
        
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        
        $this->data['page_title'] = lang('view_purchase');
        
        $this->load->view($this->theme . 'mf_purchases_packaging/view', $this->data);
        
    }
    
    function add() {  
        $this->load->model("mf_purchases_packaging_model");
        if(!$this->site->route_permission('mf_purchases_packaging_add')) {
			$this->session->set_flashdata('error', lang('access_denied'));
			redirect();
		}
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if ($item_id && $item_qty ) { 
                    ...
                    $findPkMaterialQty = $this->db->select("*")->from('mf_material_packaging')->where('id',$item_id)->get()->row();

                    if(!$findPkMaterialQty){
                        $from_qty = $item_qty;
                        $to_qty = $item_qty;
                        $balance = $item_qty;
                    }else{
                        $from_qty = $findPkMaterialQty->quantity; //previous qty
                        $balance = $from_qty + $item_qty;
                        $to_qty = $balance; 
                    }
        ...
        if ($this->form_validation->run() == true && $this->mf_purchases_packaging_model->addPurchase($data, $products)) {

            // insert into mf_packaging_material_log table for packaging report
           if(count($logData) > 0){
                foreach ($logData as $key => $item) {
                    $updateCurrentStock = $this->db->where("id",$item["material_id"])->update("mf_material_packaging",['quantity'=>$item["balance"]]);
                    // print_r($updateCurrentStock);die;
                    $insertIntoPkLog = $this->db->insert("mf_packaging_material_log",$item);
                }
           }
            ...
            $this->data['page_title'] = lang('add_purchase_packaging');
            ...
            $bc = array(
                ...
                array(
                    'link' => '#',
                    'page' => lang('add_purchase_packaging')
                )
            );
            
            $meta = array(
                'page_title' => lang('Add Packaging Material Purchase'),
                'bc' => $bc
            );
            $this->session->unset_userdata('squNo');
            // $lastProSeque = $this->mf_purchases_packaging_model->lastProSeque();
            ...
            // }else{
                $this->data['lastProSeque'] = '';
            // }
            $this->page_construct('mf_purchases_packaging/add', $this->data, $meta);
        }
    ...
    function edit($id = NULL) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $paid = $this->mf_purchases_packaging_model->getParchaseInfo($id);        
            $paid = $paid->paid;
        ...
        if ($this->form_validation->run() == true && $this->mf_purchases_packaging_model->updatePurchase($id, $data, $products)) {
            
            
            // update log regarding with match on material qty update

            if(count($logData) > 0){
                foreach ($logData as $key => $item) {
                    $this->db->select('*')->from('mf_packaging_material_log')->where("material_id",$item["material_id"])->order_by('created_at', 'desc')->limit(1);
                    $query = $this->db->get();

                    if ($query->num_rows() == 1)
                    {
                        $row = $query->row();
                        $getCurrentStock = $this->db->select("*")->from('mf_material_packaging')->where('id',$item["material_id"])->get()->row();

                        // echo "<pre>";
                        // print_r($row->balance > $getCurrentStock->quantity);
                        // die;

                        if($row->balance > $getCurrentStock->quantity){
                            // need to decrease qty current stock would be get current stock
                            $item["balance"] = $getCurrentStock->quantity;
                            $item["to_qty"]  = $getCurrentStock->quantity;
                            $item["to_sale"] = $row->balance - $getCurrentStock->quantity;
                            $item["from_qty"] = $row->balance;
                            $item["type"] = 2;
                            $item["comment"] = "Update sales decrease stock";
                            ...
                            $item["balance"] = $getCurrentStock->quantity;
                            $item["to_qty"]  = $getCurrentStock->quantity;
                            $item["add_qty"] =$getCurrentStock->quantity - $row->balance ;
                            $item["from_qty"] = $row->balance;
                            $item["type"] = 2;
                            $item["comment"] = "Update sales increase stock";
                            $item["date"]   = date("Y-m-d");
                            $insertIntoPkLog = $this->db->insert("mf_packaging_material_log",$item);
                        }
            ...
            $this->data['mf_purchase'] = $this->mf_purchases_packaging_model->getPurchaseByID($id);            
            $inv_items = $this->mf_purchases_packaging_model->getAllPurchaseItems($id);             
            ...
            foreach ($inv_items as $item) {
                $row = $this->site->getPackagingMaterialByID($item->material_id);        
            ...
            $meta = array(
                'page_title' => lang('Edit Packaging Material Purchase'),
                'bc' => $bc
            );
            
            $this->page_construct('mf_purchases_packaging/edit', $this->data, $meta);        
    ...
    function suggestions($id = NULL) {
        ...
        $rows = $this->mf_purchases_packaging_model->getProductNames($term);
    ...
    function listParPay($id) {
        ...
        $this->data['pay'] = $pay;
        $this->load->view($this->theme . 'mf_purchases_packaging/list_par_pay', $this->data);
    ...
    function add_payment($id = NULL) {
        ...
        if ($this->input->post('add_payment')) {
            ...
            $this->data['action'] = site_url('mf_purchases_packaging/add_payment/' . $id);
            
            if ($this->data['due_amount'] < 1) {
                
                $this->data['error_alert'] = ' Already payment paid ! ';
                
                $this->load->view($this->theme . 'mf_purchases_packaging/alert', $this->data);
                
            } else {
                
                $this->load->view($this->theme . 'mf_purchases_packaging/add_payment', $this->data);
            }
    ...
    function edit_payment($id) {
        ...
        if ($this->input->post('add_payment')) {
            ...
            $this->data['action'] = site_url('mf_purchases_packaging/edit_payment/' . $id);
