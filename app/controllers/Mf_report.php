...
class Mf_report extends MY_Controller
{
    ...
    public function packaging_material()
    {
        ...
        $this->data["all_month"] = get_all_months();
        $this->data["year_range"] = get_year();
    ...
    public function monthly_production()
    {
        ...
        if (isset($_GET['product']) && $_GET['select_month'] && $_GET['select_year']) {
            ...
            for ($i = 1; $i < $numDays + 1; $i++) {
                ...
                $date = $select_year . '-' . $monthFormatting . '-' . $dateFormatting;
                $getData = $this->db->select('*')->from('mf_production_log')->where(["product_id" => $productId, 'date' => $date])->order_by('date', 'desc')->limit(1);
                ...
                if ($query) {
                    if ($query->num_rows() == 1) {
                        $result = $query->row();
                        $productionLogItems[$i] =  array_merge_recursive((array) $result, ['carry' => false]);
                        $prevLog = $result;
                        $found = true;
                        if (!$this->data["products_recipe_items"])
                            $this->data["products_recipe_items"] = json_decode($result->recipe_info, true);
                    } else {
                        if ($found)
                            $productionLogItems[$i] = array_merge_recursive((array) $prevLog, ['carry' => true]);
                    }
                } else {
                    if ($found)
                        $productionLogItems[$i] = array_merge_recursive((array) $prevLog, ['carry' => true]);
        ...
        $this->data["product_log"] = $productionLogItems;

        $bc = array(array('link' => '#', 'page' => lang('reports')), array('link' => '#', 'page' => lang('Monthly Production Report')));

        $this->data["products"] = $this->db->select("*")->from("products")->get()->result();

        $this->data["all_month"] = get_all_months();

        $this->data["year_range"] = get_year();

        $query = $this->db->select('id, name')->from('mf_recipe_mst')->get();

        $recipeArr = [];

        foreach ($query->result_array() as $row) {
            $recipeArr[$row['id']] = $row['name'];
        }
        // products_recipe_items
        $this->data["all_recipe"] = $recipeArr;

        $meta = array('page_title' => lang('Monthly Production Report'), 'bc' => $bc);

        $this->page_construct('mf_report/monthly_production', $this->data, $meta);
    }


    public function exp_monthly_production()
    {
        $pkLogItems = [];

        $fileName = "monthly_production_report_" . date('Y-m-d_h_i_s') . ".xls";
        ...
        if (isset($_GET['product']) && $_GET['select_month'] && $_GET['select_year']) {
            ...
            for ($i = 1; $i < $numDays + 1; $i++) {
                ...
                if ($query) {
                    if ($query->num_rows() == 1) {
                        ...
                        $prevLog = $result;
                        $found = true;
                        if (count($recipeItems) != 1)
                            $recipeItems = json_decode($result->recipe_info, true);
                    } else {
                        if ($found)
                            $productionLogItems[$i] = array_merge_recursive((array) $prevLog, ['carry' => true]);
                    }
            ...
            $all_recipe = $recipeArr;

            if (count($recipeItems) > 0) {
                foreach ($recipeItems as $key => $recipe) {
                    $fields[] = $all_recipe[$key];
                }
            }

            $moreFields = ['ADD', 'B/F', 'TOTAL', 'TO SALE', 'BALANCE'];

            $fields = array_merge_recursive($fields, $moreFields);

            $excelData = implode("\t", array_values($fields)) . "\n";


            if ($productionLogItems) :

                foreach ($productionLogItems as $key => $value) {
                    ...
                    foreach ($recipeItemArr as $key => $v) {
                        $lineData[] = $v;
                    }
                    ...
                    $lineData = array_merge_recursive($lineData, $MoreLineData);
