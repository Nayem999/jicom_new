	...
	$(document).ready(function() {
        ...
        $("#add_item").autocomplete({
            source: base_url+'mf_purchases_packaging/suggestions',
            minLength: 1,
            autoFocus: false,
            delay: 200,
        ...
        $('#reset').click(function (e) {
            bootbox.confirm(lang.r_u_sure, function (result) {
                if (result) {
                    if (get('mf_packaging_items')) {
                        remove('mf_packaging_items');
                    }
        ...
        $(document).on("change", '.rquantity', function () {
            ...
            mf_packaging_items[item_id].row.qty = new_qty;
            store('mf_packaging_items', JSON.stringify(mf_packaging_items));
			prid = '';
            loadItems(prid);
        });
        ...
        $(document).on("change", '.rcost', function () {
            ...
            item_id = row.attr('data-item-id');
            mf_packaging_items[item_id].row.cost = new_cost;
            store('mf_packaging_items', JSON.stringify(mf_packaging_items));
            prid = '';
            loadItems(prid);
        }); 
        ...
        $(document).on("change", '.rbrand', function () {
            ...
            item_id = row.attr('data-item-id');
            mf_packaging_items[item_id].row.brand = new_brand;
            store('mf_packaging_items', JSON.stringify(mf_packaging_items));
            prid = '';
            loadItems(prid);
        }); 
...
function loadItems(prid) {
    ...
    if (get('mf_packaging_items')) {
        ...
        mf_packaging_items = JSON.parse(get('mf_packaging_items'));
		
		console.log(mf_packaging_items);
	    // console.log(all_brand);

        $.each(mf_packaging_items, function () {
            var item = this;
			
            var item_id = Settings.item_addition == 1 ? item.item_id : item.id;
            mf_packaging_items[item_id] = item;
...
function add_order_item(item) { 
    ...
    if (mf_packaging_items[item_id]) {
        mf_packaging_items[item_id].row.qty = parseFloat(mf_packaging_items[item_id].row.qty) + 1;
        //mf_packaging_items[item_id].row.set = 'dd';
    } else {
        mf_packaging_items[item_id] = item;
        mf_packaging_items[item_id].row.cost = 1;
        mf_packaging_items[item_id].row.qty = 1;
        mf_packaging_items[item_id].row.brand = 0;
    }
...
function pRemove(id,arindexid, pqtyval, prd_id) {
	...
	$.removeElementFromCollection(mf_packaging_items, arindexid);
    ...
    mf_packaging_items.splice(index, prd_id);
    store('mf_packaging_items', JSON.stringify(mf_packaging_items));
    loadItems(prid);
