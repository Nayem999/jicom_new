function add_invoice_item(item) {
    ...
    if (spositems[item_id]) {
        ...
        spositems[item_id].row.seq = '';
        spositems[item_id].row.war = '';
    }
    ...
    if (get('spositems')) {
        ...
        $.each(spositems, function () {
           ...
           // alert(item.row.seq);
            ...
            $("#control_7"+row_no).ultraselect({oneOrMoreSelected: '*'}); 
            ...
            $('#list-table-div').scrollTop(0);
             ...
             if(sQty > 0){
                 ...
                 if(Number(arrayCount) == Number(item_qty)){
                    ...
                    $('#submit-sale').removeAttr('disabled', true );
                   // $('#' + row_no).removeClass('sequence-match');
...
$(document).ready(function(){
   ...
   $(document).on("change", '.war', function () {
        var row = $(this).closest('tr');
        /*if (!is_numeric($(this).val())) {
            loadItems();
            bootbox.alert(lang.unexpected_value);
        ...
        var new_war = $(this).val(),
        item_id = row.attr('data-item-id');
        spositems[item_id].row.war = new_war;
        localStorage.setItem('spositems', JSON.stringify(spositems));
        loadItems();
    });
