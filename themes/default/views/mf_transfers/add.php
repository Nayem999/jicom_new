            ...
            <div class="box box-primary">
                ...
                <div class="box-body">
                    ...
                    <div class="col-lg-12">
                        ...
                        echo form_open_multipart("mf_transfers/add"); ?>
                                    ...
                                    <table id="transferTable" class="table table-striped table-bordered">
                                        ...
                                        <tfoot>
                                            ...
                                            <tr class="active">
                                                ...
                                                <th class="col-xs-1"></th>
                                                <input type="hidden" name="gtotal" id="g_total"/>
                        ...
                        <div class="row">
                           ...
                           <!--  <div class="col-md-4">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <?= lang('Form Factory', 'Form Factory'); ?>
                                    <p><?php  echo $from_warehouseInfo->name;?></p>
                                    <a href="javascript:;" onclick="mfTransfer()">Change Factory</a>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <?= lang('To Factory', 'To Factory'); ?>
                                    <?php
                                        $wr[''] = lang("select") . " " . lang("To Factory");
                                        foreach ($factory_stores as $k=>$v) {
                                            $wr[$v->id] = $v->name;
                                        }
                                    ?>
                        ...
                        <div class="form-group">
                            ...
                            <?= form_submit('add_transfers', lang('Add Raw Material Transfers'), 'class="btn btn-primary" id="add_transfers"'); ?>
...
<script type="text/javascript">
    ...
    $(function(){
     $("#towarehouse").change(function(){
         ...
         var url = '<?php echo base_url('mf_transfers/fn_supplierInfo') ?>'+'/'+tostore;
...
<script>
    $(document).ready(function() {
        ...
        $("#add_item").autocomplete({
            source: base_url + 'mf_transfers/suggestions',
            minLength: 1,
            autoFocus: false,
            delay: 200,
        ...
        $(document).on("change", '.rquantity', function() {
            ...
            mf_tspoitems[item_id].row.qty = new_qty;
            store('mf_tspoitems', JSON.stringify(mf_tspoitems));
            trnsid = '';
            loadItems(trnsid);
        });
        ...
        $(document).on("change", '.rcost', function() {
            ...
            var new_cost = parseFloat($(this).val()),
                ...
                item_id = row.attr('data-item-id');
            mf_tspoitems[item_id].row.cost = new_cost;
            store('mf_tspoitems', JSON.stringify(mf_tspoitems));
            trnsid = '';
            loadItems(trnsid);
        });
    ...
    function loadItems(trnsid) {
        ...
        if (get('mf_tspoitems')) {
            ...
            mf_tspoitems = JSON.parse(get('mf_tspoitems'));
            //console.log(spoitems);

            $.each(mf_tspoitems, function() {

                var item = this;
                //alert(item);
                var item_id = Settings.item_addition == 1 ? item.item_id : item.id;
                mf_tspoitems[item_id] = item;
                ...
                tr_html = '<td style="min-width:100px;"><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="brand_id[]" type="hidden" class="rid" value="' + brand_id + '"><span class="sname" id="name_' + row_no + '">' + item_name + ' (' + brand_name + ') Qty ' + store_qty + '</span>';
    ...
    function add_order_item(item) {
        ...
        if (mf_tspoitems[item_id]) {
            mf_tspoitems[item_id].row.qty = parseFloat(mf_tspoitems[item_id].row.qty) + 1;
        } else {
            mf_tspoitems[item_id] = item;
            mf_tspoitems[item_id].row.sqtrans = '';
        }
    ...
    function pRemove(id, arindexid, pqtyval) {
        ...
        //alert($('#gtotal').text());
        //spoitems.splice(2,1);
        $.removeElementFromCollection(mf_tspoitems, arindexid);
